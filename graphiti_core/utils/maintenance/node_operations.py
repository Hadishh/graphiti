...
async def resolve_extracted_nodes(
    ...
    entity_types_dict: dict[str, BaseModel] = (
    ...
    for resolution in node_resolutions:
        ...
        for idx in duplicates:
            existing_node = (
                existing_nodes[idx] if 0 <= idx < len(existing_nodes) else resolved_node
            )
...
async def extract_attributes_from_node(
    ...
    entity_attributes_model = pydantic.create_model(
        unique_model_name, **attributes_definitions
    )
...
async def dedupe_node_list(
    ...
    # Prepare context for LLM
    nodes_context = [
        {"uuid": node.uuid, "name": node.name, **node.attributes} for node in nodes
    ]
